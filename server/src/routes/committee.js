...
async function tallyFor(protocolId) {
  const votes = await db.all(`
    ...
    WHERE protocol_votes.protocol_id = $1
...
async function assignmentsFor(protocolId) {
  return db.all(`
    ...
    WHERE protocol_review_assignments.protocol_id = $1
    ORDER BY protocol_review_assignments.assigned_at, personnel.name
  `, [protocolId]);
}

async function commentsFor(protocolId) {
  return db.all(`
    ...
    WHERE protocol_review_comments.protocol_id = $1
...
async function castVote(protocolId, { personnel_id, vote, comment }) {
  ...
  await db.run(`
    ...
    ON CONFLICT(protocol_id, personnel_id) DO UPDATE SET
      vote = excluded.vote, comment = excluded.comment, voted_at = CURRENT_TIMESTAMP
  `, [protocolId, Number(personnel_id), vote, comment || null]);

  return { status: 201, body: await tallyFor(protocolId) };
}
...
router.get("/protocols", async (_req, res) => {
  ...
  for (const p of protocols) {
    withTallies.push({
      ...p,
      ...(await tallyFor(p.id)),
      assignments: await assignmentsFor(p.id),
      comments: await commentsFor(p.id),
    });
  }
...
router.get("/protocols/:id/votes", async (req, res) => {
  const protocol = await db.get("SELECT * FROM protocols WHERE id = $1", [req.params.id]);
  if (!protocol) return res.status(404).json({ error: "Protocol not found" });
  res.json({
    protocol,
    ...(await tallyFor(req.params.id)),
    assignments: await assignmentsFor(req.params.id),
    comments: await commentsFor(req.params.id),
  });
});
...
router.post("/protocols/:id/votes", async (req, res) => {
  const { status, body } = await castVote(req.params.id, req.body);
...
router.get("/protocols/:id/reviews", async (req, res) => {
  const protocol = await db.get("SELECT * FROM protocols WHERE id = $1", [req.params.id]);
  if (!protocol) return res.status(404).json({ error: "Protocol not found" });
  res.json({
    protocol,
...
router.post("/protocols/:id/comments", async (req, res) => {
  ...
  const person = await db.get("SELECT id, name FROM personnel WHERE id = $1", [Number(personnel_id)]);
  ...
  const created = await db.get(`
    ...
    VALUES ($1, $2, $3, $4)
    RETURNING id, protocol_id, personnel_id, section, comment, created_at
  `, [protocolId, Number(personnel_id), section, String(comment).trim()]);
  ...
  const full = await db.get(`
    ...
    FROM protocol_review_comments
    JOIN personnel ON personnel.id = protocol_review_comments.personnel_id
    WHERE protocol_review_comments.id = $1
  `, [created.id]);
...
router.patch("/protocols/:id/assign", async (req, res) => {
  const protocolId = req.params.id;
  ...
  await db.run(`
    ...
    VALUES ($1, $2, $3)
    ON CONFLICT(protocol_id, personnel_id) DO UPDATE SET
      role = excluded.role, assigned_at = CURRENT_TIMESTAMP
  `, [protocolId, Number(personnel_id), role]);
  ...
  const created = await db.get(`
    ...
    FROM protocol_review_assignments
    JOIN personnel ON personnel.id = protocol_review_assignments.personnel_id
    WHERE protocol_review_assignments.protocol_id = $1 AND protocol_review_assignments.personnel_id = $2
  `, [protocolId, Number(personnel_id)]);
...
router.patch("/protocols/:id/review-method", async (req, res) => {
  const protocolId = req.params.id;
  ...
  await db.run("UPDATE protocols SET review_method = $1 WHERE id = $2", [review_method, protocolId]);
  res.status(200).json(await db.get("SELECT * FROM protocols WHERE id = $1", [protocolId]));
});
