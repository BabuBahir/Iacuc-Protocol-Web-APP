// SQLite schema (dev/test backend). Kept in sync with schema.postgres.js —
// same tables, same columns, same constraints; only the dialect differs.
// Applies CREATE TABLE IF NOT EXISTS + the two migration guards for
// databases created before the summary/surgery-detail columns existed.

export function applySqliteSchema(db) {
