...
describe("species", () => {
  ...
  test("rejects an empty name", async () => {
    const res = await request(app).post("/api/admin/species").send({ name: "  " });
  ...
  test("deletes a species", async () => {
    const create = await request(app).post("/api/admin/species").send({ name: "Ferret" });
    const del = await request(app).delete(`/api/admin/species/${create.body.id}`);
    assert.equal(del.status, 204);
  });

  test("404s deleting an unknown species", async () => {
    const res = await request(app).delete("/api/admin/species/9999");
...
describe("roles", () => {
  ...
  test("rejects a duplicate role name (unique constraint)", async () => {
    await request(app).post("/api/admin/roles").send({ name: "Duplicate Role" });
    const res = await request(app).post("/api/admin/roles").send({ name: "Duplicate Role" });
    assert.equal(res.status, 409);
  });
  ...
  test(
    ...
    async () => {
      const role = await request(app).post("/api/admin/roles").send({ name: "PI" });
      await request(app)
        .post("/api/admin/personnel")
        .send({ name: "Dr. Someone", role_id: role.body.id });

      const del = await request(app).delete(`/api/admin/roles/${role.body.id}`);
  ...
  test("allows deleting a role with no personnel assigned", async () => {
    const role = await request(app).post("/api/admin/roles").send({ name: "Unused Role" });
    const del = await request(app).delete(`/api/admin/roles/${role.body.id}`);
    assert.equal(del.status, 204);
  });
});
...
describe("personnel", () => {
  ...
  test("rejects missing name or role_id", async () => {
    const res = await request(app).post("/api/admin/personnel").send({ name: "No role" });
    assert.equal(res.status, 400);
  });

  test("list includes role_name via join", async () => {
    const role = await request(app).post("/api/admin/roles").send({ name: "Co-Investigator" });
    await request(app).post("/api/admin/personnel").send({ name: "Dr. Co", role_id: role.body.id });

    const list = await request(app).get("/api/admin/personnel");
    assert.equal(list.body.length, 1);
    assert.equal(list.body[0].role_name, "Co-Investigator");
  });

  test("deletes personnel", async () => {
    const role = await request(app).post("/api/admin/roles").send({ name: "Some Role" });
    const person = await request(app)
      .post("/api/admin/personnel")
      .send({ name: "Dr. Temp", role_id: role.body.id });

    const del = await request(app).delete(`/api/admin/personnel/${person.body.id}`);
    assert.equal(del.status, 204);
  ...
  test("404s deleting unknown personnel", async () => {
    const res = await request(app).delete("/api/admin/personnel/9999");
    assert.equal(res.status, 404);
  });
});
