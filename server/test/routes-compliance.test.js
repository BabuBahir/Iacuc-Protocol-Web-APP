...
function insertRole(name, isCommittee = false) {
  const existing = db.prepare("SELECT id FROM roles WHERE name = ?").get(name);
  if (existing) return existing.id;
  return db.prepare("INSERT INTO roles (name, is_committee) VALUES (?, ?)")
    .run(name, isCommittee ? 1 : 0).lastInsertRowid;
}

function insertPerson(name, roleName = "Lab Technician") {
  const roleId = insertRole(roleName);
  return db.prepare("INSERT INTO personnel (name, role_id) VALUES (?, ?)")
    .run(name, roleId).lastInsertRowid;
}

// Training/OHSP writes are office actions (gated): carry the office actor.
const withOffice = (req) => req.set("X-Actor", OFFICE_ACTOR);
...
describe("GET /api/personnel/compliance", () => {
  beforeEach(() => {
    resetTables(db);
    seedOfficeActor(db); // clearing OHSP is an office-gated write
  });
  ...
  test("lists every person with derived compliance status", async () => {
    ...
    await withOffice(request(app).post(`/api/personnel/${a}/ohsp`)).send({ status: "Cleared" });
    db.prepare(`
    ...
    // Dr. A + Dr. B + the seeded office fixture (Maya Patel, no training/OHSP).
...
describe("POST /api/personnel/:id/training", () => {
  beforeEach(() => {
    resetTables(db);
    seedOfficeActor(db); // adding training is an office-gated write
  });

  test("adds a training record and derives its status", async () => {
    const id = await insertPerson("Dr. Eve");
    const res = await withOffice(request(app)
  ...
  test("400s when course or completed_date is missing", async () => {
    ...
    const noCourse = await withOffice(request(app).post(`/api/personnel/${id}/training`)).send({ completed_date: "2025-01-01" });
    assert.equal(noCourse.status, 400);

    const noDate = await withOffice(request(app).post(`/api/personnel/${id}/training`)).send({ course: "Course" });
    assert.equal(noDate.status, 400);
  });

  test("404s for an unknown person", async () => {
    const res = await withOffice(request(app).post("/api/personnel/9999/training")).send({ course: "Course", completed_date: "2025-01-01" });
    assert.equal(res.status, 404);
  });
});
...
describe("PATCH /api/personnel/:id/training/:trainingId", () => {
  ...
  test("updates a training record, e.g. extending an expiry", async () => {
    ...
    const created = await withOffice(request(app)
      .post(`/api/personnel/${id}/training`))
  ...
  test("404s for a record that belongs to another person or doesn't exist", async () => {
    ...
    const wrongOwner = await withOffice(request(app).patch(`/api/personnel/9999/training/${created.body.id}`)).send({ course: "X" });
    assert.equal(wrongOwner.status, 404);

    const missing = await withOffice(request(app).patch(`/api/personnel/${other}/training/9999`)).send({ course: "X" });
...
describe("DELETE /api/personnel/:id/training/:trainingId", () => {
  ...
  test("removes a training record", async () => {
    ...
    const res = await withOffice(request(app).delete(`/api/personnel/${id}/training/${created.body.id}`));
  ...
  test("404s for an unknown record", async () => {
    ...
    const res = await withOffice(request(app).delete(`/api/personnel/${id}/training/9999`));
    assert.equal(res.status, 404);
  });
});
...
describe("OHSP clearance", () => {
  ...
  test("POST upserts clearance status", async () => {
    ...
    const updated = await withOffice(request(app)
      .post(`/api/personnel/${id}/ohsp`))
...
describe("GET /api/protocols/:id/personnel", () => {
  ...
  test("computes per-person compliance and the all_compliant flag", async () => {
    ...
    await withOffice(request(app).post(`/api/personnel/${compliant}/ohsp`)).send({ status: "Cleared" });
